...
val testFederationAllTestsRequested: Boolean
    get() = testFederationSubsets == setOf(TestSubset.AllTests)

/**
 * Provides the configured test subsets, or `null` when none are configured.
 * With no subsets configured, the discovery filter does not restrict test selection. Other test filters still apply.
 */
