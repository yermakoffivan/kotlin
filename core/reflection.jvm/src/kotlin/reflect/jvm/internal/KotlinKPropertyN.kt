...
internal open class KotlinKPropertyN<out V>(
...
) : KotlinKProperty<V>(container, signature, rawBoundReceiver, kmProperty, overriddenStorage, rawBoundContextArguments = null) {
    ...
    override fun createBound(boundReceiver: Any?, boundContextArguments: List<Any?>?): ReflectKProperty<V> {
        if (boundContextArguments.isNullOrEmpty()) error("Cannot bind receiver without binding context arguments: $this")
        return when (val receiverCount = countUnboundReceivers(boundReceiver)) {
            0 -> KotlinKProperty0(container, signature, boundReceiver, kmProperty, overriddenStorage, boundContextArguments)
            1 -> KotlinKProperty1<Any?, V>(container, signature, boundReceiver, kmProperty, overriddenStorage, boundContextArguments)
...
internal class KotlinKMutablePropertyN<V>(
    ...
    override fun createBound(boundReceiver: Any?, boundContextArguments: List<Any?>?): ReflectKProperty<V> {
        if (boundContextArguments.isNullOrEmpty()) error("Cannot bind receiver without binding context arguments: $this")
        return when (val receiverCount = countUnboundReceivers(boundReceiver)) {
            0 -> KotlinKMutableProperty0(container, signature, boundReceiver, kmProperty, overriddenStorage, boundContextArguments)
            1 -> KotlinKMutableProperty1<Any?, V>(container, signature, boundReceiver, kmProperty, overriddenStorage, boundContextArguments)
            else -> throw KotlinReflectionInternalError("Cannot bind context arguments to a property with $receiverCount receivers: $this")
        }
    }
