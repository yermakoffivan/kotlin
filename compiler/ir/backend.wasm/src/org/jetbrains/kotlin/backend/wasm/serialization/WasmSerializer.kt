...
class WasmSerializer(outputStream: OutputStream) {
    ...
    fun serializeCompiledTypes(definedTypes: WasmCompiledTypesFileFragment) = with(definedTypes) {
        ...
        serializeMap(contTypes, ::serializeIdSignature, ::serializeWasmContType)
    }
    ...
    private fun serializeWasmHeapType(type: WasmHeapType) =
        when (type) {
            ...
            is ContHeapTypeSymbol -> withTag(HeapTypeTags.HEAP_CONT_TYPE) { serializeIdSignature(type.type) }
    ...
    private fun serializeWasmImmediate(i: WasmImmediate): Unit =
        when (i) {
            ...
            is ContTypeSymbol -> withTag(ImmediateTags.CONT_TYPE) { serializeIdSignature(i.type) }
