...
class WasmDeserializer(inputStream: InputStream, private val skipLocalNames: Boolean = false) {
    ...
    private fun deserializeHeapType() =
        withTag { tag ->
            when (tag) {
                ...
                HeapTypeTags.HEAP_CONT_TYPE -> ContHeapTypeSymbol(deserializeIdSignature())
    ...
    private fun deserializeImmediate(): WasmImmediate =
        withTag { tag ->
            when (tag) {
                ImmediateTags.BLOCK_TYPE_FUNCTION -> WasmImmediate.BlockType.Function(deserializeImmediate() as WasmImmediate.TypeIdx)
                ...
                ImmediateTags.CONT_TYPE -> ContTypeSymbol(deserializeIdSignature())
    ...
    fun deserializeCompiledTypesFragment() = WasmCompiledTypesFileFragment(
        ...
        contTypes = deserializeMap(::deserializeIdSignature, ::deserializeContDeclaration),
    )
